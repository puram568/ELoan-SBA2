...
public class CustomerController {
	...
	public ResponseEntity<UserDto> register(@RequestBody @Valid UserDto userDto,BindingResult result) throws InvalidDataException{
		if(result.hasErrors())
	...
	public ResponseEntity<LoanOutputDto> applyLoan(@PathVariable Long customerId,
												 @RequestBody @Valid LoanDto loanDto,BindingResult result) throws InvalidDataException{
		if(result.hasErrors())
			throw new InvalidDataException(GlobalHandler.from(result));
